...
def quick_sort(ranks, pages):
    if len(pages) == 0 or len(pages) == 1:
        ...
        i = 1
        while i < len(pages):
            if ranks[pages[i]] <= ranks[pivot]:
                left.append(pages[i])
            else:
                right.append(pages[i])
            i += 1
        left = quick_sort(ranks, left)
        right = quick_sort(ranks, right)
        result = result + left
        result.append(pivot)
        result = result + right
        return result
...
#solution
def quicksort_pages(pages, rank):
