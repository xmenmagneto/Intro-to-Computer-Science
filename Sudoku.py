...
def check_sudoku(sudoku):
    ...
    for row in sudoku:
        i = 1
        #check whether number 1-n belongs to the row.
        while i <= n:
            if i not in row:
                return False
            i += 1
    ...
    j = 1 #check index
    column = []
    while i < n: #check for one column
        for row in sudoku:
            column.append(row[i])
        i += 1
        print column
...
#solution
#every number can only appear once in each row & column
#check each row & column at the same time 
def check_sudoku(p):
  n = len(p)
