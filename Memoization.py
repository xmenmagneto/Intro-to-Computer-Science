...
def cached_execution(cache, proc, proc_input):
    ...
    if proc_input in cache:
        if proc in cache[proc_input]:
            return cache[proc_input][proc]
    else:
        value = proc(proc_input)
        cache[proc_input] = {proc: value}
        return value
...
def cached_execution(cache, proc, proc_input):
    #format of cache: {proc_input1 : proc_output2, proc_input2: proc_output2...}
